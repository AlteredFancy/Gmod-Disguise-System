SWEP.PrintName = "Passport DarkRP"
	...
	if !IsValid(vm) then return end
		...
		if (m) then
			...
			cam.Start3D2D(pos+ang:Right()*3+ang:Forward()*-1.80+ang:Up()*1.0, ang, 0.02)
			        draw.SimpleText("Name:", "PassHud2", 40, -50, Color(0,0,0,255))
					draw.SimpleText(self.Owner:Name() or "Unknown", "PassHud", 40, -40, Color(0,0,0,255))
					draw.SimpleText("Job:", "PassHud2", 40, -17, Color(0,0,0,255))
					draw.SimpleText(self.Owner:getDarkRPVar("job") or "Unknown", "PassHud3", 40, -5, Color(0,0,0,255))
					draw.SimpleText("City:", "PassHud2", 40, 15, Color(0,0,0,255))
...
if SERVER then
	function SWEP:SecondaryAttack()
		...
		owner:SelectWeapon("disguise_swep")
	end
end
...
function SWEP:Holster(wep)
	...
	if SERVER then
		...
		if IsValid(owner) and owner:IsPlayer() then
			if wep:GetClass() ~= "disguise_swep" then
